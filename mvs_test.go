	...
	for _, line := range strings.Split(tests, "\n") {
		...
		case "build":
			...
			fns = append(fns, func(t *testing.T) {
				list, err := BuildList(context.Background(), []module.Version{m(kf[1])}, reqs)
		...
		case "downgrade":
			...
			fns = append(fns, func(t *testing.T) {
				list, err := Downgrade(context.Background(), m(kf[1]), reqs, ms(kf[1:])...)
				checkList(t, key, list, err, val)
			})
			continue
		case "req":
			...
			fns = append(fns, func(t *testing.T) {
				list, err := Req(context.Background(), m(kf[1]), kf[2:], reqs)
				checkList(t, key, list, err, val)
			})
			continue
		}
...
func (r reqsMap) Upgrade(_ context.Context, m module.Version) (module.Version, error) {
...
func (r reqsMap) Previous(_ context.Context, m module.Version) (module.Version, error) {
...
func (r reqsMap) Required(_ context.Context, m module.Version) ([]module.Version, error) {
	rr, ok := r[m]
	if !ok {
