...
import (
	...
	"github.com/pgavlin/mvs/internal/par"
...
type Reqs interface {
	...
	Required(ctx context.Context, m module.Version) ([]module.Version, error)
...
type UpgradeReqs interface {
	...
	Upgrade(ctx context.Context, m module.Version) (module.Version, error)
}
...
type DowngradeReqs interface {
	...
	Previous(ctx context.Context, m module.Version) (module.Version, error)
}
...
func BuildList(ctx context.Context, targets []module.Version, reqs Reqs) ([]module.Version, error) {
	return buildList(ctx, targets, reqs, nil)
}

func buildList(ctx context.Context, targets []module.Version, reqs Reqs, upgrade func(context.Context, module.Version) (module.Version, error)) ([]module.Version, error) {
	...
	work.Do(10, func(m module.Version) {
		...
		var err error
		if m.Version != "none" {
			required, err = reqs.Required(ctx, m)
		}

		u := m
		if upgrade != nil {
			upgradeTo, upErr := upgrade(ctx, m)
			if upErr == nil {
				u = upgradeTo
			} else if err == nil {
				err = upErr
...
func Req(ctx context.Context, mainModule module.Version, base []string, reqs Reqs) ([]module.Version, error) {
	list, err := BuildList(ctx, []module.Version{mainModule}, reqs)
	if err != nil {
		return nil, err
	}
...
func UpgradeAll(ctx context.Context, target module.Version, reqs UpgradeReqs) ([]module.Version, error) {
	return buildList(ctx, []module.Version{target}, reqs, func(ctx context.Context, m module.Version) (module.Version, error) {
		if m.Path == target.Path {
			return target, nil
		}

		return reqs.Upgrade(ctx, m)
	})
}

// Upgrade returns a build list for the target module
// in which the given additional modules are upgraded.
func Upgrade(ctx context.Context, target module.Version, reqs UpgradeReqs, upgrade ...module.Version) ([]module.Version, error) {
	list, err := reqs.Required(ctx, target)
	if err != nil {
		return nil, err
	}
	...
	return buildList(ctx, []module.Version{target}, &override{target, list, reqs}, func(_ context.Context, m module.Version) (module.Version, error) {
...
// correctly.
func Downgrade(ctx context.Context, target module.Version, reqs DowngradeReqs, downgrade ...module.Version) ([]module.Version, error) {
	...
	list, err := BuildList(ctx, []module.Version{target}, reqs)
	if err != nil {
		return nil, err
	}
...
List:
	...
	actual, err := BuildList(ctx, []module.Version{target}, &override{
		target: target,
		list:   downgraded,
		Reqs:   reqs,
	})
	...
	return BuildList(ctx, []module.Version{target}, &override{
		target: target,
		list:   downgraded,
		Reqs:   reqs,
	})
...
func (r *override) Required(ctx context.Context, m module.Version) ([]module.Version, error) {
	if m == r.target {
		return r.list, nil
	}
	return r.Reqs.Required(ctx, m)
}
